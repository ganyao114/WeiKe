...
public class SaveBooksActivity extends Activity implements IAddViewOnclickCallBack,OnClickListener,OnItemClickListener{
	...
	private static class MyHandler extends Handler {
		private final WeakReference<SaveBooksActivity> mcontext;

		public MyHandler(SaveBooksActivity context) {
			mcontext = new WeakReference<SaveBooksActivity>(context);
		}
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				String errmsg = (String) msg.getData().getSerializable(
						"ErroMsg");
				((SaveBooksActivity) mcontext.get()).showTip(errmsg);
				break;
			case 1:
				@SuppressWarnings("unchecked")
				List<Books> tmplist = (List<Books>) msg.getData().getSerializable("list");
				((SaveBooksActivity) mcontext.get()).doShow(tmplist);
				break;
			default:
