...
public class ReadHistoryActivity extends Activity implements IAddViewOnclickCallBack,OnClickListener
,OnItemClickListener{
	...
	private static class MyHandler extends Handler {
		private final WeakReference<ReadHistoryActivity> mcontext;

		public MyHandler(ReadHistoryActivity context) {
			mcontext = new WeakReference<ReadHistoryActivity>(context);
		}
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				String errmsg = (String) msg.getData().getSerializable(
						"ErroMsg");
				((ReadHistoryActivity) mcontext.get()).showTip(errmsg);
				break;
			case 1:
				@SuppressWarnings("unchecked")
				List<Books> tmplist = (List<Books>) msg.getData().getSerializable("list");
				((ReadHistoryActivity) mcontext.get()).doShow(tmplist);
				break;
			default:
